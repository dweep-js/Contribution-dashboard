...
async function fetchLanguageStats(username) {
    ...
    if (!Array.isArray(repos)) {
        return { languages: [], totalStars: 0, topRepo: null, reposScanned: 0, error: "Unexpected response from GitHub" };
    }
...
function buildStats(days) {
    ...
    const weeklyAvg = sorted.length ? (sorted.reduce((s, d) => s + d.count, 0) / (sorted.length / 7)) : 0;
...
export async function fetchGitHubStats(username) {
    ...
    if (!profileResponse.ok) {
        throw new Error(profileResponse.status === 404 ? "User not found!" : "GitHub API error.");
    }
